...
Seven steps. No credential is involved at any point.
...
whenever the run lands on a weekend or before the day's price publishes — a Sunday
shows it, and so does any Monday before the day's price posts, since that is the case
collapsing them would get wrong.
...
With the contribution keys filled in (step 7) there are **two** holding rows, not one: the
anchored count, dated to the statement, and the estimate, dated to the last
...
### 7. The contribution accrual is arithmetic, not a guess
